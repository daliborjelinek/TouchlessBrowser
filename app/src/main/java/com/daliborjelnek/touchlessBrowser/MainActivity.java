...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (currentApiVersion >= Build.VERSION_CODES.M) {
            boolean permissions = verifyPermissions(this);
           if(permissions){
               setupCamera(600, 600);
           }
    ...
    public void onLandmarksAvailable(ArrayList<Point> landmarks, Bitmap bmp) {

        int SENSITIVITY_THRESHOLD = 10;
        int H_SCROLL_THRESHOLD = 50;
        int V_SCROLL_THRESHOLD = 200;
        int HEADTILT_THRESHOLD = 30;
        int NOSE_COMPENSATION = 2;
        int SMILE_THRESHOLD = 150;
        double H_SPEED = 2;
        double V_SPEED = 4;
        ...
        double vpositive, hpositive; //if delta is positive it will be 1 otherwise -1

        int deltaH = (landmarks.get(30).x - landmarks.get(2).x) - (landmarks.get(14).x - landmarks.get(30).x);
        int deltaV = (landmarks.get(30).y - landmarks.get(27).y) * NOSE_COMPENSATION - (landmarks.get(8).y - landmarks.get(30).y)+20;
        ...
        if (headtilt < -HEADTILT_THRESHOLD) {
        ...
        if (smile < SMILE_THRESHOLD && clickTriggered == true) {
        ...
        if (mPreviewDialog != null && mPreviewDialog.isShowing()) {
            ...
            mPreviewDialog.setPreviewBitmap(bmp);
            ...
            public void run() {
                ...
                if (r == true) {
                    if (mPreviewDialog.isShowing()) {
                        mPreviewDialog.showPreview();
                        mPreviewDialog.hide();
                    }
                } else {
                    mPreviewDialog.show();
                    mPreviewDialog.showLoading();
                }
